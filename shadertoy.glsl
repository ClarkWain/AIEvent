...
vec3 co = vec3(1.);
vec2 q;
float radius = 0.18, dp = 6.9, pw = 1.;
float vr1 = 1., vr2 = 1., vr3 = 1., vr4 = 1., vr5 = 1., vr6 = 1., vr7 = 1., vr8 = 1., vr9 = 1., vr10 = 1.;
float r(vec2 c, vec2 d) {
    vec2 k = abs(q - c) - 0.5 * d;
    return max(k.x, k.y);
}
float rr(vec2 c, vec2 d) {
    d -= vec2(2.0 * radius);
    return abs(length(max(abs(q - c) - 0.5 * d, 0.0)) - radius);
}
float cpu(vec2 p) {
    q = p;
    q.x *= 3.;
    ...
    float d = 1.;
    if(i == 0.)
        d = max(rr(vec2(0.), vec2(.7)), -r(vec2(0.35, 0.), vec2(1., 0.3)));
    else if(i == 1.)
        d = min(max(rr(vec2(-0.2, 0.175), vec2(1.1, .35)), -r(vec2(-1.35, 0.), vec2(2.))), max(abs(r(vec2(0.), vec2(.7))), -r(vec2(0.15, 0.), vec2(1., 2.))));
    else if(i == 2.)
        d = max(rr(vec2(0., 0.2), vec2(.7, 1.1)), -r(vec2(0., 1.35), vec2(2.)));
    return pow(clamp(d * dp, 0., 1.), pw);
}
vec3 flx(vec2 p, float ct) {
    float t;
    vec3 q = vec3(p, p.x - p.y + iTime * 0.3);
    t = abs(pn(q));
    t += .5 * abs(pn(q * 2.));
    t += .25 * abs(pn(q * 4.));
    t *= 0.5;
...
float sk(float g) {
    g = abs(g * 40.);
    return cos(g) * pow(1. - clamp(g / 11., 0., 1.), 2.);
}
float scv(float t) {
    t = clamp(t, 0., 1.);
